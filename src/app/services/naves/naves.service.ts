...
export class NavesService {
  ...
  get(id: number): Observable<any> {
    ...
    return this.http.get<INaveModel>(url, options ).pipe(
                      catchError((error: HttpErrorResponse) => {
                        if(error.status === 400){handleError();}
                        return throwError(() =>error);
                      })
  ...
  update( data: any): Observable<any> {
    ...
    return this.http.post<INaveModel>(url, data, options).pipe(
                      catchError((error: HttpErrorResponse) => {
                        if(error.status === 400){handleError();}
                        return throwError(() =>error);
                      })
  ...
  crear(data: any): Observable<any> {
    const options : any = getOptions();
    return this.http.post<INaveModel>(this.url+'/create/', data, options).pipe(
                      catchError((error: HttpErrorResponse) => {
                        if(error.status === 400){handleError();}
                        return throwError(() =>error);
                      })
